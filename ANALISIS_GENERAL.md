...
- La limitación de `tenants/$tenantId` (código de licencia como única "contraseña" del negocio, sin identidad revocable) se deja documentada como pendiente de una Cloud Function si se quiere resolver del todo — no se tocó, por ser un cambio de arquitectura, no un parche de reglas. Ver el apartado siguiente para el detalle de qué implicaría.

**Bug adicional encontrado y arreglado durante las pruebas de activación** (commit `2ae538d`): `verifyCodeIssuedOnPlatform()` no tenía ningún límite de tiempo. Si la conexión con Firebase se colgaba a medias — sin dar un error explícito, simplemente sin responder nunca — la app se quedaba esperando indefinidamente y el botón de activar licencia parecía "no hacer nada", sin ningún mensaje. El aviso de "sin conexión" ya existía en los 3 idiomas, pero en ese escenario no se llegaba a mostrar jamás. Se añade un límite de 12 segundos tras el cual se trata como "sin conexión" y sí se avisa. Es el peor momento posible para un fallo silencioso: le pasaría a un cliente nuevo en su primer contacto con el producto, sin ninguna pista de qué hacer.

### Limitación de `tenants/$tenantId` — qué implicaría resolverla

**El problema**: el `tenantId` de cada negocio se deriva matemáticamente de su código de licencia, y las reglas de Firebase solo exigen `auth != null` (una sesión anónima cualquiera) para leer y escribir en `gastrogoan/tenants/$tenantId`. En la práctica, el código de licencia **es** la contraseña permanente de todos los datos de ese negocio. Consecuencias reales:
- Si el código se filtra una sola vez (una captura, un email reenviado, un ex-empleado que lo recuerda), quien lo tenga puede leer y escribir los datos completos de ese negocio directamente contra Firebase, sin pasar por la app ni por ningún PIN.
- **No es revocable**: como el `tenantId` se deriva del código, cortar el acceso obliga a reemitir el código y migrar los datos del negocio al nuevo tenant.

**Por qué no se arregla con reglas**: las reglas de Firebase solo pueden decidir en función de lo que traiga el token de autenticación. Con sesiones anónimas no hay nada que distinga al dueño legítimo de cualquier otro — haría falta que el token llevara dentro el negocio al que pertenece, y eso solo lo puede emitir un servidor de confianza.

**Qué haría falta** (esbozo, no implementado):
1. Una Cloud Function `activarLicencia(codigo, password)` que valide el par contra `issuedCodes` **en el servidor** y devuelva un *custom token* de Firebase con un claim del tipo `{tenantId: "..."}`.
2. Que la app llame a esa función al activar, en vez de calcular el tenant en el cliente, y use `signInWithCustomToken()`.
3. Cambiar las reglas de `auth != null` a `auth.token.tenantId === $tenantId`, para que cada sesión solo pueda tocar su propio negocio.
4. Con eso, revocar un negocio pasa a ser marcar una bandera en el servidor: la siguiente petición de token se deniega, sin reemitir códigos ni migrar datos.

**Coste y contrapartidas**: las Cloud Functions requieren el plan **Blaze** de Firebase (de pago por uso; para este volumen sería de céntimos al mes, pero exige tarjeta y deja de ser "gratis del todo"). Además introduce una dependencia de red en la activación que hoy no existe de la misma forma, y hay que mantener el código desplegado. **No es urgente**: mientras los códigos se entreguen con cuidado y no se publiquen, el riesgo real es bajo. Es la mejora natural si en algún momento el producto crece a muchos negocios o se quiere poder cortar el acceso a un impago sin reemitir su licencia.

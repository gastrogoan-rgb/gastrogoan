...
const MANUAL_CHAPTERS = [
  {
    ...
    <p>Dins de Gestió Econòmica (i també a El Meu Negoci → Dades) hi ha un botó <strong>"Registre d'activitat"</strong> que obre un historial del que ha anat fent el teu equip. <strong>Només registra accions d'empleats, mai les teves com a propietari</strong> — la idea és poder veure què ha fet cada persona de l'equip, no auditar-te a tu mateix. Pots filtrar per empleat per veure només els moviments d'una persona concreta.</p>
    ...
    <p>Inside Financial Management (and also in My Business → Data) there's an <strong>"Activity log"</strong> button that opens a history of what your team has been doing. <strong>It only records employee actions, never your own as owner</strong> — the point is to see what each team member did, not to audit yourself. You can filter by employee to see only that person's actions.</p>

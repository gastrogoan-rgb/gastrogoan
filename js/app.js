...
const MANUAL_CHAPTERS = [
  {
    ...
    <p>Dentro de Gestión Económica (y también en Mi Negocio → Datos) hay un botón <strong>"Registro de actividad"</strong> que abre un historial de todo lo relevante que ha ido pasando en el negocio: quién ha hecho qué, cuándo y desde qué sesión. Puedes filtrar por empleado (o por el propietario) para ver solo los movimientos de una persona.</p>
    <p>Ahí mismo queda registrado, entre otras cosas: <strong>inicios y cierres de sesión</strong> (quién ha entrado y cuándo), anulaciones de líneas en el TPV, cierres de caja (con aviso si hay descuadre), descuentos aplicados o retirados, cambios de disponibilidad de un plato, pedidos a proveedor marcados como recibidos, turnos creados/editados/borrados y reseteos de PIN. Las acciones más delicadas para el negocio (por ejemplo, un descuadre de caja relevante, un descuento grande, un reseteo de PIN) aparecen <strong>destacadas en rojo</strong> para que las localices de un vistazo.</p>
    <p>Algunas acciones tienen además su propio historial detallado y siguen teniendo su pantalla dedicada (Anulaciones en el TPV, Historial de Arqueos, Historial de Personal): ese detalle completo sigue estando ahí igual que siempre, y el Registro de actividad simplemente añade también un resumen de esas mismas acciones para que tengas una vista única de todo el negocio sin tener que ir módulo por módulo.</p>
    <div class="manual-tip"><i class="ti ti-bulb"></i>Es la herramienta ideal si notas algo raro (una caja descuadrada, un descuento que no cuadra, un plato que apareció como agotado sin motivo) y quieres saber rápidamente quién lo hizo y cuándo.</div>`,
    ...
    <p>Dins de Gestió Econòmica (i també a El Meu Negoci → Dades) hi ha un botó <strong>"Registre d'activitat"</strong> que obre un historial de tot el que ha anat passant al negoci: qui ha fet què, quan i des de quina sessió. Pots filtrar per empleat (o pel propietari) per veure només els moviments d'una persona.</p>
    <p>Allà mateix queda registrat, entre altres coses: <strong>inicis i tancaments de sessió</strong> (qui ha entrat i quan), anul·lacions de línies al TPV, tancaments de caixa (amb avís si hi ha descaix), descomptes aplicats o retirats, canvis de disponibilitat d'un plat, comandes a proveïdor marcades com a rebudes, torns creats/editats/esborrats i reinicis de PIN. Les accions més delicades per al negoci (per exemple, un descaix rellevant, un descompte gran, un reinici de PIN) apareixen <strong>destacades en vermell</strong> perquè les localitzis d'un cop d'ull.</p>
    <p>Algunes accions tenen a més el seu propi historial detallat i continuen tenint la seva pantalla dedicada (Anul·lacions al TPV, Historial d'Arqueigs, Historial de Personal): aquell detall complet segueix estant-hi igual que sempre, i el Registre d'activitat simplement hi afegeix també un resum d'aquestes mateixes accions perquè tinguis una vista única de tot el negoci sense haver d'anar mòdul per mòdul.</p>
    <div class="manual-tip"><i class="ti ti-bulb"></i>És l'eina ideal si notes alguna cosa estranya (una caixa descaixada, un descompte que no quadra, un plat que va aparèixer com a exhaurit sense motiu) i vols saber ràpidament qui ho va fer i quan.</div>`,
    ...
    <p>Inside Financial Management (and also in My Business → Data) there's an <strong>"Activity log"</strong> button that opens a history of everything relevant that has happened in the business: who did what, when and from which session. You can filter by employee (or by the owner) to see only that person's actions.</p>
    <p>It records, among other things: <strong>logins and logouts</strong> (who signed in and when), voided lines in the POS, cash closures (with a warning if there's a mismatch), discounts applied or removed, dish availability changes, supplier orders marked as received, shifts created/edited/deleted and PIN resets. The most sensitive actions for the business (for example, a significant cash mismatch, a large discount, a PIN reset) appear <strong>highlighted in red</strong> so you can spot them at a glance.</p>
    <p>Some actions also keep their own detailed history and still have their own dedicated screen (Voids in the POS, Cash Closure History, Staff History): that full detail is still there exactly as before, and the Activity log simply also adds a summary of those same actions so you have a single view of the whole business without going module by module.</p>
    <div class="manual-tip"><i class="ti ti-bulb"></i>It's the ideal tool if you notice something odd (a mismatched cash drawer, a discount that doesn't add up, a dish that showed as unavailable for no reason) and want to quickly find out who did it and when.</div>`},
  },
  {

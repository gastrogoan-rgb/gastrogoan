...
const MANUAL_CHAPTERS = [
  ...
  },
  {
    ...
    <p>Para que el cliente sepa con seguridad si su reserva está confirmada (y no se quede con la duda), puedes activar el envío automático de un email en cuanto se confirme, cuando la modifique o cuando se cancele —tanto si fue sola como si lo hiciste tú a mano—. El email de confirmación lleva un enlace para que el propio cliente pueda cambiar la hora o cancelar sin llamarte. Se configura en <strong>Mi Negocio → Confirmación de reservas por email</strong>, con una cuenta gratuita tuya de <strong>EmailJS</strong> (no hace falta programar nada). Aquí va el resumen; dentro de esa tarjeta hay un botón <strong>"¿Cómo lo configuro? Guía paso a paso"</strong> con el detalle completo:</p>
    ...
    <div class="manual-step"><div class="sn">3</div><div class="st">En <strong>"Email Templates" → "Create New Template"</strong>, crea la plantilla de <strong>confirmación</strong>: en "To Email" pon <code>{{to_email}}</code>, entra en <strong>"Edit Content" → "Code Editor"</strong> y escribe el mensaje con estas variables tal cual: <code>{{client_name}}</code>, <code>{{business_name}}</code>, <code>{{date}}</code>, <code>{{time}}</code>, <code>{{people}}</code>, <code>{{table_name}}</code> y <code>{{manage_link}}</code> (este último es el enlace para cambiar la hora o cancelar — sin él en el texto, el cliente no lo recibe). Guarda y copia el <strong>Template ID</strong> de la pestaña "Settings".</div></div>
    <div class="manual-step"><div class="sn">4</div><div class="st">Repite el paso anterior para crear una <strong>segunda plantilla</strong>, la de <strong>modificación</strong> (mismas variables): avisa al cliente de que su reserva YA confirmada ha cambiado de fecha/hora, con un texto tipo "Tu reserva ha sido modificada" en vez de "está confirmada" — si reutilizas la de confirmación aquí, el cliente se confunde al recibir dos veces "confirmada". Copia su <strong>Template ID</strong>.</div></div>
    <div class="manual-step"><div class="sn">5</div><div class="st">Repite otra vez para crear una <strong>tercera plantilla</strong>, la de <strong>cancelación</strong> (mismas variables, sin <code>{{manage_link}}</code>). Copia también su <strong>Template ID</strong> — los tres son distintos entre sí.</div></div>
    <div class="manual-step"><div class="sn">6</div><div class="st">En <strong>"Account" → "API Keys"</strong>, copia tu <strong>Public Key</strong> (no el "Private Key" de al lado).</div></div>
    <div class="manual-step"><div class="sn">7</div><div class="st">Pega los 5 códigos en <strong>Mi Negocio → Confirmación de reservas por email</strong>, activa el interruptor y guarda. Pulsa los tres botones de prueba (<strong>"Enviar email de prueba"</strong>, <strong>"Probar modificación"</strong> y <strong>"Probar cancelación"</strong>) con tu propio email para comprobar que los tres llegan.</div></div>
    ...
    <p>Perquè el client sàpiga amb seguretat si la seva reserva està confirmada (i no es quedi amb el dubte), pots activar l'enviament automàtic d'un email en confirmar-se, en modificar-la o en cancel·lar-se —tant si va ser sol com si ho vas fer tu a mà—. L'email de confirmació porta un enllaç perquè el mateix client pugui canviar l'hora o cancel·lar sense trucar-te. Es configura a <strong>El Meu Negoci → Confirmació de reserves per email</strong>, amb un compte gratuït teu d'<strong>EmailJS</strong> (no cal programar res). Aquí va el resum; dins d'aquesta targeta hi ha un botó <strong>"Com ho configuro? Guia pas a pas"</strong> amb el detall complet:</p>
    ...
    <div class="manual-step"><div class="sn">3</div><div class="st">A <strong>"Email Templates" → "Create New Template"</strong>, crea la plantilla de <strong>confirmació</strong>: a "To Email" posa <code>{{to_email}}</code>, entra a <strong>"Edit Content" → "Code Editor"</strong> i escriu el missatge amb aquestes variables tal qual: <code>{{client_name}}</code>, <code>{{business_name}}</code>, <code>{{date}}</code>, <code>{{time}}</code>, <code>{{people}}</code>, <code>{{table_name}}</code> i <code>{{manage_link}}</code> (aquest últim és l'enllaç per canviar l'hora o cancel·lar — sense ell al text, el client no el rep). Desa i copia el <strong>Template ID</strong> de la pestanya "Settings".</div></div>
    <div class="manual-step"><div class="sn">4</div><div class="st">Repeteix el pas anterior per crear una <strong>segona plantilla</strong>, la de <strong>modificació</strong> (mateixes variables): avisa el client que la seva reserva JA confirmada ha canviat de data/hora, amb un text tipus "La teva reserva ha estat modificada" en lloc de "està confirmada" — si reutilitzes la de confirmació aquí, el client es confon en rebre dues vegades "confirmada". Copia el seu <strong>Template ID</strong>.</div></div>
    <div class="manual-step"><div class="sn">5</div><div class="st">Repeteix una altra vegada per crear una <strong>tercera plantilla</strong>, la de <strong>cancel·lació</strong> (mateixes variables, sense <code>{{manage_link}}</code>). Copia també el seu <strong>Template ID</strong> — els tres són diferents entre si.</div></div>
    <div class="manual-step"><div class="sn">6</div><div class="st">A <strong>"Account" → "API Keys"</strong>, copia la teva <strong>Public Key</strong> (no el "Private Key" del costat).</div></div>
    <div class="manual-step"><div class="sn">7</div><div class="st">Enganxa els 5 codis a <strong>El Meu Negoci → Confirmació de reserves per email</strong>, activa l'interruptor i desa. Prem els tres botons de prova (<strong>"Enviar email de prova"</strong>, <strong>"Provar modificació"</strong> i <strong>"Provar cancel·lació"</strong>) amb el teu propi email per comprovar que arriben tots tres.</div></div>
    ...
    <p>So the customer knows for sure whether their reservation is confirmed (instead of being left wondering), you can turn on an automatic email when it's confirmed, when it's modified, and when it's cancelled — whether that happened on its own or you did it by hand. The confirmation email carries a link so the customer can change the time or cancel without calling you. Set it up in <strong>My Business → Reservation confirmation by email</strong>, with your own free <strong>EmailJS</strong> account (no coding needed). Here's the summary; inside that card there's a <strong>"How do I set this up? Step-by-step guide"</strong> button with the full detail:</p>
    ...
    <div class="manual-step"><div class="sn">3</div><div class="st">Under <strong>"Email Templates" → "Create New Template"</strong>, build the <strong>confirmation</strong> template: set "To Email" to <code>{{to_email}}</code>, open <strong>"Edit Content" → "Code Editor"</strong> and write the message with these variables as-is: <code>{{client_name}}</code>, <code>{{business_name}}</code>, <code>{{date}}</code>, <code>{{time}}</code>, <code>{{people}}</code>, <code>{{table_name}}</code> and <code>{{manage_link}}</code> (this last one is the link to change the time or cancel — without it in the text, the customer won't get it). Save it and copy the <strong>Template ID</strong> from the "Settings" tab.</div></div>
    <div class="manual-step"><div class="sn">4</div><div class="st">Repeat the previous step to create a <strong>second template</strong>, the <strong>modification</strong> one (same variables): it tells the customer their ALREADY confirmed reservation changed date/time, with wording like "Your reservation has been changed" instead of "is confirmed" — reusing the confirmation one here confuses the customer, who'd get "confirmed" twice. Copy its <strong>Template ID</strong>.</div></div>
    <div class="manual-step"><div class="sn">5</div><div class="st">Repeat once more to create a <strong>third template</strong>, the <strong>cancellation</strong> one (same variables, without <code>{{manage_link}}</code>). Copy its <strong>Template ID</strong> too — all three are different from each other.</div></div>
    <div class="manual-step"><div class="sn">6</div><div class="st">Under <strong>"Account" → "API Keys"</strong>, copy your <strong>Public Key</strong> (not the "Private Key" next to it).</div></div>
    <div class="manual-step"><div class="sn">7</div><div class="st">Paste the 5 codes into <strong>My Business → Reservation confirmation by email</strong>, turn the switch on, and save. Click all three test buttons (<strong>"Send test email"</strong>, <strong>"Test modification"</strong> and <strong>"Test cancellation"</strong>) with your own email to check all three arrive.</div></div>

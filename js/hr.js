  ...
  box.innerHTML = `
    ...
    <div class="toolbar">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        ${allEmps.length ? `<button class="owner-strict btn" onclick="openHorarioFijoModal()"><i class="ti ti-repeat"></i> ${t('title.fixedSchedule')}</button>` : ''}
...
function horarioFijoDiaRowHtml(i, dia){
  const tipoOptions = Object.entries(SHIFT_TYPES).map(([k,v]) => `<option value="${k}"${k===dia.tipo?' selected':''}>${k} - ${v.label}</option>`).join('');
  const noHorario = ['D','V','B'].includes(dia.tipo);

...
| Tablet/desktop | El grid de "Mesas" (la acción más usada de toda la app: tocar una mesa para abrirla) aparecía **por debajo** de las tarjetas de estadísticas del día y los botones de acciones (Lista de espera, Modo caos, Anulaciones...), obligando a hacer scroll en una tablet de 768px de alto antes de ver ninguna mesa. | Molesto | En una tablet real usada en horizontal, el usuario entraba a TPV y lo primero que veía eran cifras de "Ventas hoy: 0,00€" en vez de las mesas — tenía que bajar para poder trabajar. | ✅ **Arreglado**: `js/tpv.js`, `renderTPV()` — las Mesas (y el selector de carta, avisos de última hora, pedidos online pendientes) pasan a mostrarse primero; las estadísticas del día y la barra de botones de acción (Modo caos, Anulaciones, Cierre de caja...) se mueven al final. Verificado visualmente en tablet horizontal y móvil: las mesas ya son lo primero que se ve al entrar, sin hacer scroll. |
...
1. **Grid de Mesas visible sin scroll en TPV** (tablet) — ✅ ya aplicado esta sesión. Es la pantalla que un camarero abre decenas de veces por turno; que la acción principal estuviera "escondida" bajo estadísticas del día daba sensación de app mal pensada aunque todo lo demás estuviera bien.
...
- `js/tpv.js` — `renderTPV()` reordenado: Mesas (y avisos urgentes/pedidos online pendientes) pasan a mostrarse primero; estadísticas del día y barra de botones de acción, al final. Verificado visualmente en tablet y móvil, y con los tests existentes (`smoke.test.mjs`, `audit-active.mjs`) en verde tras el cambio.
